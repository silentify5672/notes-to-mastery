...
def generate_flashcards(notes_text):
    ...
    num_cards = max(3, min(word_count // 100, 20))

    system_prompt = f"You are a study assistant. Extract key concepts from the given notes and turn them into approximately {num_cards} flashcards, covering the material thoroughly without excessive repetition. Respond ONLY with a JSON array like this: [{{\"question\": \"...\", \"answer\": \"...\", \"topic\": \"...\"}}]. No extra text, just the JSON."
...
st.write("Paste your notes below, or upload a PDF, and I'll turn them into flashcards.")
...
if "flashcards" in st.session_state:
    ...
    if not st.session_state["quiz_done"] and index < len(flashcards):
        card = flashcards[index]
        st.write(f"**Question {index + 1} of {len(flashcards)}:** {card['question']}")
        ...
        st.success(f"📊 Quiz complete! {score['correct']} correct, {score['incorrect']} incorrect, {score['idk']} idk — {percentage}%")
        st.write("**Weak topics:**", st.session_state["weak_topics"])
...
if st.session_state.get("quiz_done", False):
    ...
    if len(weak_topics_list) == 0:
        st.divider()
        st.success("🎉 No weak topics — great job, nothing to review!")
    else:
        st.divider()
        ...
        if not st.session_state["tb_done"] and topic_index < len(weak_topics_list):
            ...
            st.balloons()
            st.success("🎉 Teach-back complete! Great work reviewing your weak topics.")